...
output$totalDXACostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_dxa_cost/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("dxa costs, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
    color = "blue", fill = T, width = NULL
  )
})

output$totalDXAS1CostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_dxa_cost_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("dxa costs, 2020-", inp_year, sep = "", collapse = NULL)
...
output$totalMedCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_med_cost/1000000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("dxa costs, 2020-", inp_year, sep = "", collapse = NULL)
...
output$totalMedS1CostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_med_cost_s1/1000000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("dxa costs, 2020-", inp_year, sep = "", collapse = NULL)
...
output$totalDirectCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_direct_cost/1000000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("dxa costs, 2020-", inp_year, sep = "", collapse = NULL)
...
output$totalDirectS1CostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_direct_cost_s1/1000000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("dxa costs, 2020-", inp_year, sep = "", collapse = NULL)
...
output$totalPrimaryFracsBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_fractures_wo_previous_fracture
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary fracs, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryS1FracsBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_fractures_wo_previous_fracture_s1
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary s1 fracs, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryFracsBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_fractures_with_previous_fracture
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary fracs, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryS1FracsBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_fractures_with_previous_fracture_s1
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary s1 fracs, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$grand_total_with_prev_frac/1000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary fracs cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryS1CostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$grand_total_with_prev_frac_s1/1000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary s1 fracs cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$grand_total_wo_prev_frac/1000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary fracs cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryS1CostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$grand_total_wo_prev_frac_s1/1000000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary s1 fracs cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
###
output$totalPrimaryDXACostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_dxa_cost_wo_prev_frac/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary fracs DXA cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryS1DXACostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_dxa_cost_wo_prev_frac_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary s1 fracs DXA cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryDXACostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_dxa_cost_with_prev_frac/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary fracs DXA cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryS1DXACostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_dxa_cost_with_prev_frac_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary s1 fracs DXA cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryMedCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_med_cost_wo_prev_frac/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary fracs Med cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryS1MedCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_med_cost_wo_prev_frac_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary s1 fracs Med cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryMedCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_med_cost_with_prev_frac/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary fracs Med cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryS1MedCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_med_cost_with_prev_frac_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary s1 fracs Med cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
###
output$totalPrimaryFracCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_frac_cost_wo_prev_frac/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary fracs trt cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalPrimaryS1FracCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_frac_cost_wo_prev_frac_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("primary s1 fracs trt cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryFracCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_frac_cost_with_prev_frac/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary fracs trt cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
...
output$totalSecondaryS1FracCostBox <- renderInfoBox({
  base_case <- simulation_data$sim
  total_frax <- 0
  duration <-  as.integer(substring(input$endYear, 1, 4)) - 2020 + 1
  for(i in 1:duration) {
    total_frax <- total_frax + base_case[[i]]$total_frac_cost_with_prev_frac_s1/1000
  }
  inp_year <- as.Date(input$endYear, "%Y")
  inp_year <- format(inp_year, "%Y")
  title_text <- paste("secondary s1 fracs trt cost, 2020-", inp_year, sep = "", collapse = NULL)
  infoBox(
    title = title_text,
    value = formatC(round(total_frax), format = 'd', big.mark=','),
    icon = icon("list"),
